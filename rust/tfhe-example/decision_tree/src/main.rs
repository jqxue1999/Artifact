...
fn main() {
    ...
    println!("\n{}", "=".repeat(80));
    println!("TFHE Decision Tree Evaluation");
    println!("{}\n", "=".repeat(80));
    ...
    match mode {
        "real" => {
            ...
            println!("Running actual decision tree evaluation on encrypted data\n");

            // Display security verification first
            verify_and_print_security();

            run_real_tree_benchmarks();
        }
        "security" => {
            verify_and_print_security();
            println!();
...
fn print_help() {
    ...
    println!("Modes:");
    println!("  real      - Run real decision tree evaluation (default)");

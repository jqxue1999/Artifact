...
fn benchmark_cp_mul_6bit() -> std::time::Duration {
    ...
    let encrypted_a = FheUint6::try_encrypt(15u8, &client_key).unwrap();
    let encrypted_b = FheUint6::try_encrypt(20u8, &client_key).unwrap();
    let encrypted_c = FheUint6::try_encrypt(10u8, &client_key).unwrap();
...
/// Benchmark 6-bit mul_cp: a * (b < c)
fn benchmark_mul_cp_6bit() -> std::time::Duration {
    let config = ConfigBuilder::default().build();
    let (client_key, server_keys) = generate_keys(config);

    let encrypted_a = FheUint6::try_encrypt(10u8, &client_key).unwrap();
    let encrypted_b = FheUint6::try_encrypt(15u8, &client_key).unwrap();
    let encrypted_c = FheUint6::try_encrypt(20u8, &client_key).unwrap();
    ...
    let encrypted_cmp_6: FheUint6 = encrypted_cmp.cast_into();
    let _encrypted_product = &encrypted_a * &encrypted_cmp_6;
    let duration = start.elapsed();
...
/// Benchmark Floyd-Warshall by actually running the required number of operations
pub fn benchmark_floyd_warshall_actual(node_count: usize, bit_width: u8) -> Duration {
    let analysis = analyze_floyd_warshall(node_count);
    
    match bit_width {
        6 => benchmark_floyd_actual_6bit(analysis.total_cp_mul_operations, analysis.total_mul_cp_operations),
        8 => benchmark_floyd_actual_8bit(analysis.total_cp_mul_operations, analysis.total_mul_cp_operations),
        16 => benchmark_floyd_actual_16bit(analysis.total_cp_mul_operations, analysis.total_mul_cp_operations),
        _ => panic!("Unsupported bit width: {}", bit_width),
    }
}

/// Actually run the required number of 6-bit operations
fn benchmark_floyd_actual_6bit(cp_mul_count: usize, mul_cp_count: usize) -> Duration {
    let config = ConfigBuilder::default().build();
    let (client_key, server_keys) = generate_keys(config);
    set_server_key(server_keys);
    ...
    // Run mul_cp operations (additions + comparisons)
    ...
    // Run cp_mul operations (comparisons + conditional updates)
...
fn benchmark_floyd_actual_8bit(cp_mul_count: usize, mul_cp_count: usize) -> Duration {
    let config = ConfigBuilder::default().build();
    let (client_key, server_keys) = generate_keys(config);
    set_server_key(server_keys);
...
fn benchmark_floyd_actual_16bit(cp_mul_count: usize, mul_cp_count: usize) -> Duration {
    let config = ConfigBuilder::default().build();
    let (client_key, server_keys) = generate_keys(config);
    set_server_key(server_keys);
...
/// Run comprehensive Floyd-Warshall analysis with actual operation counting
pub fn run_real_analysis() {
    println!("\nFloyd-Warshall Algorithm Results (Actual Operation Counting):");
    
    // Test different node counts and bit widths
    let test_node_counts = vec![16, 32, 64, 128];
    let bit_widths = vec![6, 8, 16];
    
    for &node_count in &test_node_counts {
        for &bit_width in &bit_widths {
            println!("Running {} nodes, {}-bit...", node_count, bit_width);
            let actual_time = benchmark_floyd_warshall_actual(node_count, bit_width);
            println!("Floyd-Warshall: {} nodes, {}-bit: {}", 
                     node_count, bit_width, format_duration(actual_time));
        }
    }
    ...
    for &node_count in &test_node_counts {
        ...
        println!("  Additions: {}", analysis.addition_count);

...
fn main() {
    ...
    println!("\n{}", "=".repeat(80));
    println!("TFHE Floyd-Warshall Algorithm");
    println!("{}\n", "=".repeat(80));
    ...
    match mode {
        "real" => {
            ...
            println!("Running actual Floyd-Warshall algorithm on encrypted data\n");

            // Display security verification first
            verify_and_print_security();

            run_real_floyd_benchmarks();
        }
        "security" => {
            verify_and_print_security();
            println!();
...
fn print_help() {
    ...
    println!("Modes:");
    println!("  real      - Run real Floyd-Warshall algorithm (default)");
